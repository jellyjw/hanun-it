...
export class ArticleTranslator {
  ...
  private protectCodeAndTerms(text: string): {
    ...
    protectedText = protectedText.replace(
      ...
      (match, property, value) => {
        if (
          this.techTerms.includes(property.toLowerCase()) ||
          this.techTerms.includes(value.trim().toLowerCase())
        ) {
  ...
  async translateArticle(article: {
    ...
    try {
      ...
      const { protectedText: protectedTitle, placeholders: titlePlaceholders } =
        this.protectCodeAndTerms(article.title);

      const [translatedTitle, translatedContent, translatedDescription] =
        await Promise.all([
