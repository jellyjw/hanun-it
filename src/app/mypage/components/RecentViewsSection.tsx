...
export default function RecentViewsSection() {
  const { data, isLoading, error } = useQuery<ViewedArticlesResponse>({
    queryKey: ['mypage', 'views'],
    queryFn: async () => {
      const response = await fetch('/api/users/me/views?limit=3');
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold">최근 읽은 글</h3>
